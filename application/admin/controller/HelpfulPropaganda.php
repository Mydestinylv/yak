...
use app\admin\action\HelpfulPropagandaAction;
use app\common\controller\Admin;
use think\Request;
use think\Log;
class HelpfulPropaganda extends Admin
{
    /**
     * 显示资源列表
     */
    public function index(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param, 'app\admin\validate\HelpfulPropaganda.index');
            if ($result !== true) {
                return format($result);
            }
            $transfer = HelpfulPropagandaAction::index($param);
            if (!$transfer->status) {
    ...
    public function save(Request $request)
    {
        try {
            ...
            $transfer = HelpfulPropagandaAction::save($param);
            if (!$transfer->status) {
    ...
    public function read(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\admin\validate\HelpfulPropaganda.read');
            if ($result !== true) {
                return format($result);
            }
            $transfer = HelpfulPropagandaAction::read($param);
            if (!$transfer->status) {
    ...
    public function update(Request $request)
    {
        try {
            ...
            $transfer = HelpfulPropagandaAction::update($param);
            if (!$transfer->status) {
    ...
    public function delete(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\admin\validate\HelpfulPropaganda.delete');
            if ($result !== true) {
                return format($result);
            }
            $transfer = HelpfulPropagandaAction::delete($param);
            if (!$transfer->status) {
                $message = $transfer->message ?: '保存更新的资源失败';
                if($this->environment==='test'){
                    return format($message, 400, array_merge($transfer->data, [__FILE__ . ' ' . __LINE__]));
