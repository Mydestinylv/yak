...
use app\admin\action\CardManageAction;
use app\common\controller\Admin;
use think\Request;
use think\Log;

class CardManage extends Admin
{
    /**
     * 显示资源列表
     */
    public function index(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param, 'app\admin\validate\CardManage.index');
            if ($result !== true) {
                return format($result);
            }
            $transfer = CardManageAction::index($param);
            if (!$transfer->status) {
    ...
    public function save(Request $request)
    {
        try {
            $param = $request->post();
            $result = $this->validate($param, 'app\admin\validate\CardManage.save');
            if ($result !== true) {
                return format($result);
            }
    ...
    public function read(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param, 'app\admin\validate\CardManage.read');
            if ($result !== true) {
                return format($result);
            }
    ...
    public function update(Request $request)
    {
        try {
            $param = $request->post();
            $result = $this->validate($param, 'app\admin\validate\CardManage.update');
            if ($result !== true) {
                return format($result);
            }
            $transfer = CardManageAction::update($param);
            if (!$transfer->status) {
    ...
    public function delete(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param, 'app\admin\validate\CardManage.delete');
            if ($result !== true) {
                return format($result);
            }
    ...
    {
        return $this->fetch('cardManage/lists');
    }
}
