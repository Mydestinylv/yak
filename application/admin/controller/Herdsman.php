...
use app\admin\action\HerdsmanAction;
use app\common\controller\Admin;
use think\Request;
use think\Log;
class Herdsman extends Admin
{
    /**
     * 显示资源列表
     */
    public function index(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param, 'app\admin\validate\Herdsman.index');
            if ($result !== true) {
                return format($result);
            }
            $transfer = HerdsmanAction::index($param);
            if (!$transfer->status) {
    ...
    public function save(Request $request)
    {
        try {
            ...
            $transfer = HerdsmanAction::save($param);
            if (!$transfer->status) {
    ...
    public function read(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\admin\validate\Herdsman.read');
            if ($result !== true) {
                return format($result);
            }
            $transfer = HerdsmanAction::read($param);
            if (!$transfer->status) {
    ...
    public function update(Request $request)
    {
        try {
            ...
            $transfer = HerdsmanAction::update($param);
            if (!$transfer->status) {
    ...
    public function delete(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\admin\validate\Herdsman.delete');
            if ($result !== true) {
                return format($result);
            }
            $transfer = HerdsmanAction::delete($param);
            if (!$transfer->status) {
                $message = $transfer->message ?: '保存更新的资源失败';
                if($this->environment==='test'){
                    return format($message, 400, array_merge($transfer->data, [__FILE__ . ' ' . __LINE__]));
    ...
    public function password_reset(Request $request)
    {
        try {
            $param = $request->post();
            $result = $this->validate($param,'app\admin\validate\Herdsman.password_reset');
            if ($result !== true) {
                return format($result);
            }
            $transfer = HerdsmanAction::password_reset($param);
            if (!$transfer->status) {
                $message = $transfer->message ?: '保存更新的资源失败';
                if($this->environment==='test'){
                    return format($message, 400, array_merge($transfer->data, [__FILE__ . ' ' . __LINE__]));
