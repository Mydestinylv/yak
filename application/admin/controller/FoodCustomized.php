...
use app\admin\action\FoodCustomizedAction;
use app\common\controller\Admin;
use think\Request;
use think\Log;
class FoodCustomized extends Admin
{
    /**
     * 显示资源列表
     */
    public function index(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param, 'app\admin\validate\FoodCustomized.index');
            if ($result !== true) {
                return format($result);
            }
            $transfer = FoodCustomizedAction::index($param);
            if (!$transfer->status) {
    ...
    public function save(Request $request)
    {
        try {
            $param = $request->post();
            $result = $this->validate($param,'app\admin\validate\FoodCustomized.save');
            if ($result !== true) {
                return format($result);
            }
            $transfer = FoodCustomizedAction::save($param);
            if (!$transfer->status) {
    ...
    public function read(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\admin\validate\FoodCustomized.read');
            if ($result !== true) {
                return format($result);
            }
    ...
    public function update(Request $request)
    {
        try {
            $param = $request->post();
            $result = $this->validate($param,'app\admin\validate\FoodCustomized.update');
            if ($result !== true) {
                return format($result);
            }
    ...
    public function delete(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\admin\validate\FoodCustomized.delete');
            if ($result !== true) {
                return format($result);
            }
    ...
    public function feedback(Request $request)
    {
        try {
            ...
            $transfer = FoodCustomizedAction::feedback($param);
            if (!$transfer->status) {
                $message = $transfer->message ?: '添加反馈失败';
                if($this->environment==='test'){
                    return format($message, 400, array_merge($transfer->data, [__FILE__ . ' ' . __LINE__]));
                }else{
                    Log::error([__CLASS__ . '  ' . __FUNCTION__,
