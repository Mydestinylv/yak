...
use app\move\action\NewsAction;
use think\Request;
use think\Log;
class News extends App
{
    ...
    public function index(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param, 'app\move\validate\News.index');
            if ($result !== true) {
                return format($result);
            }
            $transfer = NewsAction::index($param);
            if (!$transfer->status) {
    ...
    public function save(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\move\validate\News.save');
            if ($result !== true) {
                return format($result);
            }
            $transfer = NewsAction::save($param);
            if (!$transfer->status) {
    ...
    public function read(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\move\validate\News.read');
            if ($result !== true) {
                return format($result);
            }
            $transfer = NewsAction::read($param);
            if (!$transfer->status) {
    ...
    public function update(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\move\validate\News.update');
            if ($result !== true) {
                return format($result);
            }
            $transfer = NewsAction::update($param);
            if (!$transfer->status) {
    ...
    public function delete(Request $request)
    {
        try {
            $param = $request->param();
            $result = $this->validate($param,'app\move\validate\News.delete');
            if ($result !== true) {
                return format($result);
            }
            $transfer = NewsAction::delete($param);
            if (!$transfer->status) {
                $message = $transfer->message ?: '保存更新的资源失败';
                if($this->environment==='test'){
                    return format($message, 400, array_merge($transfer->data, [__FILE__ . ' ' . __LINE__]));
