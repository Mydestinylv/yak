...
require_once(dirname(dirname(dirname(dirname(__FILE__))))."/extend/aliyun-dysms-php-sdk/api_sdk/vendor/autoload.php"); //加载文件路径修改
...
class Phonecode extends Controller
{
     ...
     * @return stdClass
     */
    public static function sendBatchSms($NameCode,$phone,$Label) {
        ...
        $request->setPhoneNumberJson(json_encode($phone, JSON_UNESCAPED_UNICODE));

        // 必填:短信签名-支持不同的号码发送不同的短信签名
        $request->setSignNameJson(json_encode($Label, JSON_UNESCAPED_UNICODE));

        // 必填:短信模板-可在短信控制台中找到
        $request->setTemplateCode("SMS_137780297");

        // 必填:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
        // 友情提示:如果JSON中需要带换行符,请参照标准的JSON协议对换行符的要求,比如短信内容中包含\r\n的情况在JSON中需要表示成\\r\\n,否则会导致JSON在服务端解析失败
        $request->setTemplateParamJson(json_encode($NameCode, JSON_UNESCAPED_UNICODE));
