...
class ReceivingAddressTask
{
    protected static $model = 'app\common\model\ReceivingAddress';
     ...
     * $join_file string 'file'
     * $action string 'LEFT or RIGHT or INNER'
     * $where array  ['login_status' => 2]
     * $field string 'sum(login_status), id, token'
     * $group string 'id'
     * */
    public static function join($table, $join_file, $action, $where, $field, $order = 'create_time desc',$paginate = 0 ,$select = 'paginate')
    {
        //读配置文件
        if (!$paginate) {
            $config = config('paginate');
