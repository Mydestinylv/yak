...
class FeedbackTask
{
    protected static $model = 'app\common\model\Feedback';
    ...
    {
        $model = new self::$model();
        $status = $model->allowField(True)->save($data, $where);
        if ($status === false) {
            Log::alert([__CLASS__ . '  ' . __FUNCTION__,
                'failure_desc' => '更新数据失败',
     ...
     * $group string 'id'
     * */
    public static function join($table, $join_file, $action, $where, $field, $order,$paginate = 0)
    {
        if (!$paginate) {
            $config = config('paginate');
            $paginate = $config['list_rows'];
