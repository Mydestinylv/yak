...
class LogisticsOrderTask
{
    protected static $model = 'app\common\model\LogisticsOrder';
     ...
     * $group string 'id'
     * */
    public static function Mjoin($table, $join_file, $action, $where, $field, $order = 'create_time desc',$select = 'paginate')
    {
        $model = (self::$model)::alias('a')
            ->join($table[0], $join_file[0], $action[0])
            ->join($table[1], $join_file[1], $action[1])
            ->join($table[2], $join_file[2], $action[2])
            ->where($where)
            ->field($field)
            ->order($order)
            ->$select();
