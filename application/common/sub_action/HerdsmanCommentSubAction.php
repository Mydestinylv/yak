...
use app\common\model\HerdsmanComment;
use app\common\task\HerdsmanCommentTask;

class HerdsmanCommentSubAction
{
    /**
     * 显示资源列表
     */
    public static function index($param)
    {
        ...
        $field = ['a.id,a.score,a.comment_content,a.create_time,b.real_name,b.tel,c.yaks_name,c.yaks_tag,d.name as herdsman_name,d.tel as herdsman_tel,e.task_name,e.task_detail'];
        $order = ['a.create_time'];
        $transfer = HerdsmanComment::alias('a')
            ->join('Customer b', 'a.customer_id = b.id', 'LEFT')
            ->join('Yaks c', 'a.yaks_id = c.id', 'LEFT')
            ->join('Herdsman d', 'a.herdsman_id = d.id', 'LEFT')
            ->join('TaskManage e', 'a.task_id = e.id', 'LEFT')
            ->field($field)
            ->where($where)
            ->order($order)
            ->paginate();
    ...
    public static function update($param)
    {
        $where['id'] = $param['id'];
        unset($param['id']);
        $transfer = HerdsmanCommentTask::update($param,$where);
        if(!$transfer->status){
    ...
    {
        $transfer = HerdsmanCommentTask::delete($param['id']);
        if(!$transfer->status){
